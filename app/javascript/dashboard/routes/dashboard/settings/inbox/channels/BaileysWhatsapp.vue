...
const createChannelAndRequestQr = async () => {
  ...
  try {
    ...
    const { data } = await BaileysChannel.requestQrCode(response.id, {
      sync_full_history: syncFullHistory.value,
      import_groups: importGroups.value,
    });
    if (data.qr_code) {
      qrCodeData.value = data.qr_code;
      sessionState.value = SESSION_STATES.QR_PENDING;
    ...
    >
      ...
      <div class="mt-4 flex flex-col gap-3">
        <label class="flex items-center gap-2 text-sm text-n-slate-12">
          ...
          <span>{{ $t('INBOX_MGMT.ADD.BAILEYS.IMPORT_HISTORY') }}</span>
        </label>
        <label class="flex items-center gap-2 text-sm text-n-slate-11">
          <input type="checkbox" :checked="false" disabled />
          <span>
            {{ $t('INBOX_MGMT.ADD.BAILEYS.IMPORT_GROUPS') }}
            <strong>{{ $t('INBOX_MGMT.ADD.BAILEYS.COMING_SOON') }}</strong>
          </span>
        </label>
      </div>
